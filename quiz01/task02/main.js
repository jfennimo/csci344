...
// I don't understand why the page isn't updating...

const statusToHTML = statusObj => {
  return `
  <div class="card">
    <h1>Name: ${statusObj.screen_name}</h1>
    <p>Status: ${statusObj.text}</p>
    <button>more</button>
    </div>`;
...
const userToHTML = userObj => {
  if(userObj.verified == true) {
    return `
  <div class="card">
    <img src=${userObj.profile_image_url}>
    <h2>${userObj.name}</h2> <i class="fa-solid fa-circle-check"></i>
    <p>${userObj.followers_count}</p>
    <p>${userObj.friends_count}</p>
    </div>`;
  }
  else {
    return `
